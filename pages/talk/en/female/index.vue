...
export default {
  ...
  methods: {
    ...
    async submitQ() {
      ...
      try {
        ...
        const response = await this.$axios.$post(
          `/rcms-api/1/en/female?_lang=en&text=${this.questionEntered}`
        );
    ...
    async processQuestion(question) {
      try {
        const response = await this.$axios.$post(`/rcms-api/1/en/female?_lang=en&text=${question}`);
        const base64EncodedData = response.mp3.audioContent;
        const decodedData = window.atob(base64EncodedData);
        const uint8Array = new Uint8Array(decodedData.length);
        for (let i = 0; i < decodedData.length; i++) {
  ...
  mounted() {
    const firstQuestion = "You are an English teacher. Greet your students and throw one simple question for free talk.";
